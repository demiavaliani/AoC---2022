...
const findDuplicates = () => {
	for (let i = 0; i < rucksack.length; i += 3) {
		...
		group.push(rucksack[i], rucksack[i + 1], rucksack[i + 2]);

		firstBag = group[0];
		secondBag = group[1];
		thirdBag = group[2];

		[...firstBag].forEach((letter) => {
			if (
				secondBag.includes(letter) &&
				thirdBag.includes(letter) &&
				!alreadyIncludes.includes(letter)
			) {
				if (letter == letter.toUpperCase()) {
					sum += 26 + (parseInt(letter, 36) - 9);
				} else {
					sum += parseInt(letter, 36) - 9;
				}

				alreadyIncludes += letter;
			}
		});
	}
